    ...
    <v-card-text class="pl-3">
      ...
      <div class="float-right mr-3">
        <v-btn @click="onLike" icon class="mr-3">
          <div>
            <v-icon :color="reviewInfo.like ? 'red' : ''">mdi-heart</v-icon>
            <p>{{ reviewInfo.like_cnt }}</p>
          </div>
        </v-btn>
        <v-btn icon>
          <v-icon color="dark darken-2" class="float-right mr-3">mdi-message-text</v-icon> {{ reviewInfo.comment_cnt }}
        </v-btn>
      </div>
